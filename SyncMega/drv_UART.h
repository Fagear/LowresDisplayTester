...
#define UART_OUTPUT_BUF_LEN		UART_OUT_LEN	// For transmitting buffer.
	...
	#error Buffer size more than 65535 (16-bit) is now supported! (UART_INPUT_BUF_LEN)
#endif
#if UART_OUTPUT_BUF_LEN>65535U
	#error Buffer size more than 65535 (16-bit) is now supported! (UART_OUTPUT_BUF_LEN)
#endif
...
void UART_TXint_enable(void);					// Enable interrupt on successful transmit.
void UART_RXint_enable(void);					// Enable interrupt on successful receive.
void UART_TXint_disable(void);					// Disable interrupt on successful transmit.
void UART_RXint_disable(void);					// Disable interrupt on successful receive.
